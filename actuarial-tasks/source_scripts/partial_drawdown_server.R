...
# Output Functions - Drawdowns --------------------------------------------
  output$pd_text_drawdown_total_withdrawals_life_ex <- renderText({
    pd_inputs = pd_inputs()
    average = Drawdown_Mean_Withdrawals_Life_Ex(Drawdown_Withdrawals = pd_withdrawals_drawdown_reactive(),
...
# Output Functions - Table and Plot ------------------------------------------------
  ...
  output$pd_plot_income_compare <- renderPlotly({
    ...
    fig <- plot_ly(table_df, x = ~round(table_df$years, 2), y = ~round(table_df$annuity_total_paid), name = "100% Annuity", type = "scatter", mode = "lines", line = list(color = 'blue', width = 4))
    fig <- fig %>% add_trace(y = ~round(table_df$drawdown_mean_withdrawn), name = '100% Drawdown',line = list(color = 'red', width = 4))
    fig <- fig %>% add_trace(y = ~round(table_df$buy_later_total), name = 'Drawdown and Subsequent Annuity Purchase', line = list(color = 'green', width = 4))
    fig <- fig %>% add_trace(y = ~round(table_df$deferred_total), name = 'Drawdown and Deferred Annuity', line = list(color = 'orange', width = 4))
    fig <- fig %>% layout(shapes = list(plotly_vline(x = pd_inputs$pd_age[2] - pd_inputs$pd_age[1], colour = 'orange'), plotly_vline(x = pd_life_ex_reactive(), colour = 'yellow')))
