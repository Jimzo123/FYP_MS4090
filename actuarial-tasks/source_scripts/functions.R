...
# SORP Functions ----------------------------------------------------------
  ...
  SORP_Table_Contributions <- function(SORP_Contributions, freq){
    ...
    SORP_Contributions <- datatable(SORP_Contributions, options = list(scrollX = TRUE, scrollY = "360px", paging = FALSE, searching = FALSE, info = FALSE, columnDefs = list(list(className = 'dt-center', targets = "_all"))), rownames= FALSE)
...
# Drawdown Functions ------------------------------------------------------
  ...
  Drawdown_Plot_Sims <- function(Drawdown_Paths, freq, n_sims = 25, points = NULL, colour = NULL){
    ...
    if (!is.null(points) && !is.null(colour)){
      points = unlist(points)
      for (i in 1:length(points)){
        p <- p + geom_vline(xintercept = points[i], color = colour[i])
      }
    }
    p <- p + 
...
# Partial Drawdown Functions ----------------------------------------------
  ...
  Partial_Drawdown_Comparison_Table <- function(age_1, age_2, freq, annuity_payment, Drawdown_Paths, Drawdown_Withdrawals, buy_later_average_annuity_payment, BuyLater_Paths, BuyLater_Withdrawals, Deferred_Paths, Deferred_Withdrawals, series = NULL, points = NULL, colour = NULL, lifetable = ILT15_female_reduced){
    ...
    table <- datatable(table_df, container = container, options = list(scrollX = TRUE, paging = FALSE, searching = FALSE, columnDefs = list(list(className = 'dt-center', targets = "_all"))), rownames = FALSE) %>%
